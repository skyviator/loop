...
School Admin child creation now uses the authenticated `create_child_with_enrollment` security-invoker RPC: it validates an active same-school school/branch/classroom, inserts the minimal child record and initial current enrollment in one transaction, and returns only the new child ID. A failed enrollment, plan-limit check, or authorization check leaves no archived orphan, default consent, or audit residue. The existing atomic `move_child_enrollment` RPC preserves the previous enrollment as history and now also refuses a destination under an inactive branch. Deactivating a child leaves its enrollment/history untouched; active child status remains an independent operational-access requirement. Multiple guardian links can coexist; revoking one link does not delete an Auth identity or revoke another child's independent link. Legal name and date of birth are supported by the schema but remain deliberately deferred from this MVP management UI to avoid expanding its privacy footprint.

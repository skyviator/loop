...
- Child creation with an initial classroom uses a narrow authenticated security-invoker RPC. It checks active same-school structure, uses existing RLS and serialized plan limits, and commits child/enrollment/default media consent/reduced audits together or rolls all of them back. Child deactivation preserves enrollment history while Teacher/Guardian operational access fails closed; reactivation never changes a separately revoked guardian link.

...
Web Push writes are asynchronous. Database triggers append minimal event metadata after authoritative attendance, message, important-announcement, or ready-photo writes. After a message commits, its Server Action passes the existing dispatcher directly to Vercel `waitUntil()` and returns without making push delivery part of message success. PostgreSQL remains the durable source of truth: Supabase Cron also invokes a narrow Vercel worker endpoint every minute through `pg_net`, so due work does not depend on another user mutation. Delivery claims atomically recalculate current guardian links, memberships, classroom assignments, feature availability, and user preferences, then send a generic lock-screen payload with only a same-origin relative route. Claims use a two-minute lease around the dispatcher's 15-second Web Push timeout; abandoned `sending` rows become retryable while active claims remain protected by `FOR UPDATE SKIP LOCKED`. Temporary failures retain bounded exponential backoff. External Web Push is at-least-once rather than exactly-once, so the stable non-sensitive per-delivery notification tag reduces visible duplicates after an ambiguous retry.
...
Messaging uses one guardian-to-school thread per guardian/child pair. PostgreSQL RLS recalculates access from active school/structure records, current enrollments, guardian links, memberships, and classroom assignments. Only the open thread subscribes to a private Supabase Realtime Broadcast topic (`message-thread:<thread UUID>`), authorized through `realtime.messages`; the browser does not subscribe to school-wide messages. Because Supabase caches channel authorization for an existing connection, Broadcast carries only a constant invalidation signal and the browser refetches message bodies through RLS before rendering them.
